...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (!email || !password || !name || !company) return json({ ok: false, error: "Name, company, email and password are required." }, 400);
    ...
    const companyId = randomId("co_");
    const userId = randomId("user_");
    const now = new Date().toISOString();
    const hp = await hashPassword(password);
    await env.DB.batch([
      env.DB.prepare("INSERT INTO auth_companies (id,name,created_at,updated_at) VALUES (?1,?2,?3,?4)").bind(companyId, company, now, now),
