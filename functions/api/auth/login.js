...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const row = await env.DB.prepare(`SELECT u.*, c.name AS company_name FROM auth_users u LEFT JOIN auth_companies c ON c.id=u.company_id WHERE u.email=?1`).bind(email).first();
    if (!row || row.status !== "Active") return json({ ok: false, error: "Invalid email or password." }, 401);
